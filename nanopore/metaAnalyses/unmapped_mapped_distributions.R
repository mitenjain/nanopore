...
if (length(unmapped[,1]) >= 1 && length(mapped[1,]) >= 1) {
    ...
    mapped.outlier <- apply(mapped, 2, repoutliers)
    unmapped.outlier <- apply(unmapped, 2, repoutliers)

    mapped.density <- density(mapped.outlier[,1], na.rm=T)
    unmapped.density <- density(unmapped.outlier[,1], na.rm=T)
    xmax <- max(unmapped.density$x, mapped.density$x)
    ymax <- max(unmapped.density$y, mapped.density$y)
    plot(mapped.density, xlim=c(0,xmax), ylim=c(0,ymax), xlab="Read Length", main=paste(analysis, "Mappable and Unmappable Read Lengths (Outliers Removed)", sep=" "), col="blue")
