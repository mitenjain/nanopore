...
if (length(unmapped[,1]) >= 1 && length(mapped[,1]) >= 1) {
    ...
    m <- hist(mapped[,1], breaks = seq(1,b+100,100), plot=F)
    u <- hist(unmapped[,1], breaks = seq(1,b+100,100), plot=F)
    ...
    legend("top", pch=15, legend=c(paste("Mapped n =", dim(mapped)[1]), paste("Unmapped n =", dim(unmapped)[1])), col=c(rgb(1,0,0),rgb(0,0,1)))
    ...
    text(y=lengths+0.015*sum(lengths), x=q, labels=paste(formatC(round(100*lengths/sum(lengths),3), format="f", digits=2), "%"), xpd=T)
