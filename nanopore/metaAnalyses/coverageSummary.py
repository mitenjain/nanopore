...
class CoverageSummary(AbstractMetaAnalysis):
    ...
    def write_file_analyze(self, entries, name, multiple_read_types=False):
        ...
        outf = open(path, "w")
        outf.write(",".join(["Name", "Mapper", "ReadFile", "ReferenceFile",  "AvgReadCoverage", "AvgReferenceCoverage", "AvgIdentity", "AvgMismatchesPerReadBase", "AvgDeletionsPerReadBase", "AvgInsertionsPerReadBase", "NumberOfMappedReads", "NumberOfUnmappedReads", "NumberOfReads"])); outf.write("\n")
        ...
        for entry, n in izip(entries, names):
            outf.write(",".join([n, entry.mapper, entry.readFastqFile, entry.referenceFastaFile,
