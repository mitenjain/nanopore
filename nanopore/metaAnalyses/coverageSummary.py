...
class CoverageSummary(AbstractMetaAnalysis):
    ...
    def run(self):
        ...
        for mapper in tmp_data:
            ...
            tmp2 = open(os.path.join(self.getLocalTempDir(), "tmp2.csv"), "w")
            tmp2.write(",".join([mapper] + tmp_data[mapper])); tmp2.write("\n")
            system("Rscript nanopore/metaAnalyses/coveragePlots.R {} {}".format(os.path.join(self.getLocalTempDir(), "tmp2.csv"), os.path.join(self.outputDir, "coverage_summary_plots_%s.pdf" % mapper)))
            tmp2.close()
        tmp.close()
