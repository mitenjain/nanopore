...
class MarginAlignMetaAnalysis(AbstractMetaAnalysis):
    def run(self):
        ...
        for readType, mapper in product(readTypes, self.mappers):
            outf = open(os.path.join(self.outputDir, "tmp.tsv"), "w")
            ...
            for algorithm in variantCallingAlgorithms:
                for proportionHeldOut in proportionsHeldOut:
                    for coverage in coverageLevels:
                        ...
                        outf.write("{0}\t{1}\t{2}\n{0}\t{1}\t{3}\n".format(str(algorithm), str(proportionHeldOut),"\t".join(map(str,falsePositiveRatesByProbability)), "\t".join(map(str,truePositiveRatesByProbability))))
            outf.close()
            system("Rscript nanopore/metaAnalyses/ROC_marginAlign.R {} {} {} {}".format(os.path.join(self.outputDir, "tmp.tsv"), os.path.join(self.outputDir, readType + "_" + mapper.__name__ + "_ROC_curves.pdf"), len(variantCallingAlgorithms), len(proportionsHeldOut)))