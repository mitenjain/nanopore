...
#since dist is uneven we need to know the largest # of columns
cols <- max(count.fields(args[2], sep=","))
dist <- read.table(args[2], fill=T, sep=",", col.names = paste("V",seq_len(cols)))
...
if (dim(dist)[2] > 2) {
	...
	legend(x="topleft", y="topleft", col=r, legend=dist[,1], lty=1, cex=0.3)
