        ...
        for reference in self.referenceFastaFiles:
            ...
            for read in self.reads:
                if read.is_mapped:
                    ...
                    unmapped.write("{}\n".format(len(read.seq)))
            unmapped.close(); mapped.close()
            if os.path.getsize(os.path.join(self.getLocalTempDir(), os.path.basename(reference) + "_unmapped")) > 0 and os.path.getsize(os.path.join(self.getLocalTempDir(), os.path.basename(reference) + "_mapped")) > 0:
