	...
	def run(self, kmer_size=5):
		for readType in self.readTypes:
			...
			if len(unmapped) > 1:
				...
				system('blastn -outfmt "7 qseqid sseqid sscinames stitle" -db nt -max_target_seqs 1 -query {} -out {}'.format(os.path.join(self.outputDir, "unmapped.fasta"), os.path.join(self.outputDir, readType + _"blast_out.txt")))
